...
class AdTableCleaner:
    ...
    def __init__(self):
        ...
        self.dir_raw_data = os.path.abspath(os.path.join(dir_atual, '..', 'dat_raw'))
        self.dir_cleaned_data = os.path.abspath(os.path.join(dir_atual, '..', 'dat_clean'))
    ...
    def clean_ad_tabela_despachos(self):    
        ...
        desp_df['coord'] = desp_df['coord_despacho'].apply(lambda x: str(x).split('/')[1] if '/' in str(x) else None)
        desp_df['divisao'] = desp_df['coord_despacho'].apply(lambda x: str(x).split('/')[-1] if '/' in str(x) else None)

        # Padronizando despachos
        desp_df.loc[desp_df.despacho == 'Processo Indeferido e Finalizado', 'despacho'] = 'Processo Indeferido e Encerrado'
    ...
    def clean_ad_tabela_comuniqueses(self):       
        ...
        # Criando colunas para valores de coordenadoria e divisão
        comun_df['coord'] = comun_df['coord_comuniquese'].apply(lambda x: str(x).split('/')[1] if '/' in str(x) else None)
    ...
    def clean_table(self):
        ...
        print('AD_CLEANER - Tabela principal do Aprova Digital criada com sucesso')
        self.clean_ad_tabela_despachos()
        print('AD_CLEANER - Tabela de despachos do Aprova Digital criada com sucesso')
        self.clean_ad_tabela_comuniqueses()
        print('AD_CLEANER - Tabela de comunique-ses do Aprova Digital criada com sucesso')
